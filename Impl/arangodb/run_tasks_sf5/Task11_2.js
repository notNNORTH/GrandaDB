res_insert = db._query(`
FOR s in Site 
FILTER s.properties.type == "roadnode"
INSERT s INTO Site_roadnode
`)
db.Site_roadnode.ensureIndex({ type: "geo", fields: ["geometry"], geoJson:true})
db.Site_roadnode.ensureIndex({ type: "persistent", fields: ["site_id"]})
print("index finish")
res1 = db._query(`
// FOR eqk IN Earthquake FILTER eqk.earthquake_id == 41862
FOR eqk IN Earthquake FILTER eqk.earthquake_id == 44340
...
LET A = (FOR fGps IN (FOR g IN Gps
         ...
         LET temp = (FOR s in Site_roadnode
