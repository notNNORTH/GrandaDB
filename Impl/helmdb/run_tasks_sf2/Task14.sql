...
INSERT INTO T14A
SELECT floor(t1.timestamp / 8) as date, t1.timestamp, t1.latitude, t1.longitude, avg(t2.pm10) AS pm10_avg
...
FROM (
	SELECT T14C.date, T14C.timestamp, (
		...
		ORDER BY ST_Distance(ST_GeomFromGeoJSON(Site_centroid.data->>'centroid'), T14C.coordinates::geography) ASC
		LIMIT 1
	) AS site_id
	FROM T14C
